...
- **Study buddy** — a pixel-art capybara in a firefly costume paces along the bottom of the side panel and the fullscreen timer. It walks the width of the screen, turns around at the far end and walks back; its legs alternate between two poses and the body dips a pixel on the stride. The costume — glowing wings, antennae and a lantern on its tail — takes the colour of the current mode, while the fur stays warm brown in every theme. When the timer is stopped it halts where it stands, shuts its eyes and lets a "z" drift up. Built from a character grid into inline SVG: no image files, no requests. Turn it off with "Capybara study buddy" in the settings.
