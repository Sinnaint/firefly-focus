...
- **Study buddy** — a pixel-art capybara in a firefly costume keeps you company in the side panel and the fullscreen timer, and reacts to the timer in three poses. While it runs she paces the width of the screen, turning around at each end, legs crossing mid-stride. Stop the timer and she crouches and sits back on her haunches; leave it stopped and she slumps down into a loaf, rests her chin on her paws and starts snoozing with a "z" drifting up. The costume — glowing wings and antennae — takes the colour of the current mode, while the fur stays warm brown in every theme. Built from character grids into inline SVG: no image files, no requests. Turn it off with "Capybara study buddy" in the settings.
- **Three timer faces** — the countdown can be drawn as the **Ring** (unchanged), **Digits only** (no ring at all, just large numerals over a hairline of progress) or a **Breathing circle** (a soft glowing halo that slowly expands and contracts while the timer runs, and holds still when paused). Applies to the side panel and fullscreen; the floating widget always uses the ring.
