...
8. **Капібара-компаньйон** (`studyBuddyEnabled`) — inline-SVG у `BUDDY_SVG`/`renderBuddyInto()` (shared.js), один екземпляр розмітки на панель і повний екран, стилі й анімації в `sidepanel.css`. Хутро навмисно **не** зав'язане на тему (це персонаж, а не елемент інтерфейсу), костюм бере `--accent`. Стан спить/не спить читається з `body[data-running]`. У віджеті його немає.
