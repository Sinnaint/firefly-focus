...
8. **Капібара-компаньйон** (`studyBuddyEnabled`) — піксель-арт-спрайт, який ходить туди-сюди по низу таймера. Тільки панель і повний екран; у плаваючому віджеті його немає (там свій CSS усередині content script).
   Спрайт **не намальований руками в SVG**, а зібраний із символьних ґраток у `shared.js`: `BUDDY_BODY` (19 рядків × 32 стовпці), `BUDDY_LEGS` (дві пози ніг) і `BUDDY_WING`. Одна клітинка = одна одиниця viewBox, `shape-rendering="crispEdges"`. Літери позначають **CSS-класи**, не кольори, тому палітра лишається в `sidepanel.css`. Правити вигляд = правити рядки ґратки; **усі рядки мають бути однакової довжини** (32 і 12), інакше спрайт поїде. `buddyRects()` склеює однакові сусідні клітинки в один `<rect>` — ~76 вузлів замість кількох сотень.
   Хутро навмисно **не** зав'язане на тему (це персонаж, а не елемент інтерфейсу), костюм (крила, вусики, ліхтарик) бере `--accent`.
   **Хода**: `.buddy` — це доріжка на всю ширину хоста, а `.buddy-walk` усередині їде `transform`-ом від краю до краю (`alternate`). Розворот — окрема анімація `buddyFace` на `.buddy-art` з **удвічі довшим** періодом і перемиканням `scaleX` на 50%: саме тоді хода завертає. Змінити тривалість = правити `--buddy-walk`, обидві анімації рахуються від неї.
   Ноги — два кадри, що чергуються через `opacity` (`buddyStep` + від'ємна затримка на другому), тіло просідає на 1px саме на кадрі з розкроком. На паузі (`body[data-running="false"]`) хода **не скидається на початок**, а `animation-play-state: paused` — капібара зупиняється там, де стояла, заплющує очі й пускає «z».
   `--buddy-size` іде і в `width`, і в `height`, тому задавати його **тільки** у `vw`/`px` — відсоток порахувався б від різних осей. У повному екрані значення живуть на `.fs-body` (щоб `.fs` могла зарезервувати смугу `--fs-buddy-lane`), а тоді мапляться на `.fs-buddy`: власне оголошення `.buddy` перебиває успадковане.
