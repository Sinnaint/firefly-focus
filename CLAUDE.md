...
5. **Ліміти даних**: ≤30 задач, текст ≤90 символів, дедлайн — рядок `YYYY-MM-DD` або `null` (`normalizeDeadline`). Дедлайни: протерміновано = червоне (`overdue`), ≤2 днів = бурштинове (`soon`) — `deadlineUrgency()` живе в shared.js, але **продубльована** у floating_widget.js (content script не бачить shared.js). Міняти логіку — в обох місцях.
